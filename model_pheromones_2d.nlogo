...
turtles-own [
  status               ;; variable telling the turtle what it is currently doing (string form, differentiates between trails)
                       ;;                   statuses: "at nest", "return-from-1", "return-from-2", "foraging-for-1", "foraging-for-2", "foraging"
...
to setup
  ...
  [ set size 0.28         ;; should be 0.28 when to scale
    ...
    ;orient West and the rotate right between 0 and 180 degrees
    ;this corresponds to ants having a preferred nest entrance direction
    set heading 0
    ;right random 180
  ]
  ;; red = not carrying food
  resize-world -70 70 -70 70
  ifelse random-dist-1 = true
  [ set dist1 (random 1000) / 1000
    if dist1 < 0.12 [set dist1 0.12]]
  [ set dist1 distance-1]
  set sign one-of [1 -1]
  ifelse random-dist-2 = true
  [set dist2 (random 1000) / 1000
    if dist2 < 0.12 [set dist2 0.12]]
  [ set dist2 distance-2]
  setup-patches
  reset-ticks
end
...
to setup-food  ;; patch procedure
  ;; setup food source one on the left
  let food-1-x (sin(-45) * dist1) * max-pxcor
  let food-1-y (cos(45) * dist1) * max-pycor + nest-location

  if (distancexy food-1-x food-1-y < 1)  [set food-source-number 1]
  ;; setup food source two on the right
  let food-2-x (sin(45) * dist2) * max-pxcor
  let food-2-y (cos(45) * dist2) * max-pycor + nest-location

  if (distancexy food-2-x food-2-y < 1)  [set food-source-number 2]
end
...
to recolor-patch  ;; patch procedure
  ...
  ifelse nest?
  [ set pcolor violet ]
  [if food-source-number = 1 [ set pcolor 127 ]
    if food-source-number = 2 [ set pcolor 67 ]]
    ;; scale color to show chemical concentration
  if nest? = false and food-source-number = 0
  [if (chemical1 + chemical2) > 0
   [ifelse (max list chemical1 chemical2) = chemical2
    [set pcolor scale-color green chemical2 0.05 15]
    [set pcolor scale-color magenta chemical1 0.05 15]]]
end
...
to go  ;; forever button
 ...
 [
  ...
  [ set chemical1 chemical1 * (100 - evaporation-rate) / 100  ;; slowly evaporate chemical
    ...
    set chemical chemical1 + chemical2
    recolor-patch ]
  tick
end
...
to return-to-nest  ;; turtle procedure
  ifelse nest?
    [ ;; drop food and head out again
      ...
      set binStatus 20       ;this resets the binary status of the turtles so that they are foraging w/o a trail
    ...
    [if prevfood = 1 [set chemical1 chemical1 + low-quality-pheromone]    ; this drops chemical proportionately with the quality of the food that the turtle previously picked up (now logged in prevfood)
      if prevfood = 2 [set chemical2 chemical2 + low-quality-pheromone * pheromone-ratio]
      uphill-nest-scent]
end
...
to look-for-food  ;; turtle procedure
  if food-source-number = 1
    [set color blue                             ; change the color of the turtle
     ...
     set binStatus 1                          ; sets binary status to full of food, returning to nest
  ...
  if food-source-number = 2
    [set color blue                            ; change the color of the turtle to reflect a different food source
     ...
     set binStatus 11                         ; sets binary status to full of food, returning to nest
  ...
  if length patch_list = 0 [rt 180
   ...
   if length patch_list = 1                              ;if there is one patch in the list, choose it
    [let patch_i item 0 patch_list
     let ci ([chemical + chemical2] of patch_i)
...
"" ""
PENS
"Quality = 1" 1.0 0 -10022847 true "" "plot count turtles with [status = \"return-from-1\" or status = \"foraging-for-1\"]"
"Quality = 2" 1.0 0 -8330359 true "" "plot count turtles with [status = \"return-from-2\" or status = \"foraging-for-2\"]"
"Total" 1.0 0 -7500403 true "" "plot count turtles with [status = \"return-from-1\" or status = \"foraging-for-1\" or status = \"return-from-2\" or status = \"foraging-for-2\"] "
